       ...
       // [ProducesResponseType(StatusCodes.Status202Accepted)]
        ...
        public async Task<ActionResult<IEnumerable<DealDto>>> Search([FromBody]SearchDealsFilter filter, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await Mediator.Send<IEnumerable<DealDto>>(new GetDealsQuery(filter), cancellationToken);
                return Ok(result);
            }
        ...
        [HttpGet]
        [Route("Deals/{id}")]
        public async Task<ActionResult<IEnumerable<DealDto>>> Get(Guid id, CancellationToken cancellationToken)
        {
            try
            {
                var result = await Mediator.Send<DealDto>(new GetOneDealQuery() { Id = id }, cancellationToken);
                return Ok(result);
            }
            catch (Exception exception)
            {
